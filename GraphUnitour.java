   ...
   public int shortestPath(GraphUnitour t[],String src, String dest,int avo)
    {
     ...
     if(avo==-1)
     {
      ...
      else
      {
         ...
         {
           for(j=0;j<n-2;j++)            //To calculate from source to all n-1 vertices
          {
           u= this.min(dist,set);       //Calculate index of vertex having Minimum distance from labelled node 
           //jTextArea1.append(""+u+"\n");
           set[u]=true;                 //Set that index as labelled node
           for(int v=0;v<n;v++)
           {
             ...
             /* Update dist[v] only if is not a labelled node set[v] should be false, there is an edge from u to v, and total weight of
              path from source node 's' to v through u is smaller than current value of dist[v] */
         	if(!set[v] && (v!=avo && v!=24) && t[u].getDistance(v)!=0 && dist[u]!=Integer.MAX_VALUE && (dist[u]+t[u].getDistance(v))<dist[v])
         	{
         		dist[v]=dist[u]+t[u].getDistance(v);
         		parent[v]=u;   //Update the parent...the parent of vertex at index v is index u
         	}
    ...
    private void initComponents() {
        ...
        jLabel6.setText("Are you Handicapped ?");
        getContentPane().add(jLabel6);
        jLabel6.setBounds(120, 240, 230, 22);
        ...
        jButton3.setBounds(300, 290, 120, 30);
        ...
        jButton4.setBounds(500, 290, 100, 30);
	...
	t[14]=new GraphUnitour(n,"Rajeev Chowk");
	  ...
	  /* Graph for Class */
	     ...
	     t[14].add(14,12,40,"Move 40 Steps towards Direction Of Statue\n");
             ...
             t[15].add(15,24,24,"While Facing Towards G1, Move 24 Steps Straight through the left Gallery\n");
             ...
             t[24].add(24,15,24,"Move 24 Steps Left , then again take left\n");
         ...
         else if((handicap.equalsIgnoreCase("yes") && (s<15 && d>=15) && avo!=24) || (handicap.equalsIgnoreCase("yes") && (s>=15 && d<15) && avo!=24))
         {
         
         for(i=0;i<n;i++)
         {
      	  if(t[i].getClasss().equals("Lift"))
          lift=i;
         }
         
            min_path=shortestPath(t,m,"Lift",avo);
            displayPath(t,s,lift);
            min_path+=shortestPath(t,"Lift",j,avo);
            displayPath(t,lift,d);
            jTextArea1.append("\nNo. of Steps from "+m+" to "+j+" is "+min_path);
         }
         else if(handicap.equalsIgnoreCase("yes") && avo==24)
         {
             jTextArea1.setText("It's Mandatory that if you're Specially Abled, you can't avoid Lift\n");
         }
         else
         {
